...
public class FailsaveCommand extends DefaultDebug {
    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull String alias, @NotNull Arguments args) {
        if (args.isEmpty()) {
            sender.sendMessage("§cThe plugin failed to load correctly. Please use /" + alias + "§c debug to create a debug paste and send it to the developer.");
            return;
        }
        ...
        sender.sendMessage("§cThe plugin failed to load correctly. Please use §a/" + alias + "§c debug to create a debug paste and send it to the developer.");
    }
}
